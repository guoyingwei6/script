...
{
	printf Ofile "Query-name\tLetter\tQueryX\tQueryY\tSbjctX\tSbjctY\tLength\tScore\tE-value\tOverlap/total\tIdentity\tSbject-Name\tQuery_sequence\tSbjct_seq\tSbjct_description\n";
}
	...
	if (/Query= (\S+)/)
	{
		if($i==1)
		{
			...
			if($score >$Score && $expect<=$Expect && $over>=$Identity && $identity2 >= $Length && $query_num <= $Query_num && $sbjct_num <= $Sbjct_num)
			{
				...
				if($Sbjct_description eq "y")
				{
					print Ofile "$query\t$letter\t$qbegin\t$qend\t$sbegin\t$send\t$length\t$score\t$expect\t$ovalap_total\t$over\t$name\t$query_seq\t$Sbjct_seq\t$annotation\n";
				}
				elsif($Sbjct_description eq "n")
				{
