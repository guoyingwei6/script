...
def base2file(varfile, sample, region, regions_file, outfile):
    ...
    if os.path.exists(outfile):
        ...
        with open(outfile, 'a') as f:
            f.write(f'>{sample}\n')
    if regions_file:
        os.system(f"bcftools query -f '[%TGT]' -s {sample} -R {regions_file} {varfile} | sed -e 's/\.\/\./NN/g' -e 's/\///g' | fold -w80 >> {outfile}")
    elif region:
        os.system(f"bcftools query -f '[%TGT]' -s {sample} -r {region} {varfile} | sed -e 's/\.\/\./NN/g' -e 's/\///g' | fold -w80 >> {outfile}")
    else:
        os.system(f"bcftools query -f '[%TGT]' -s {sample} {varfile} | sed -e 's/\.\/\./NN/g' -e 's/\///g' | fold -w80 >> {outfile}")
